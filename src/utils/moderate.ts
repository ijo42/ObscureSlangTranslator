    ...
    if (moderators.size === 0) {
        ...
        bot.onText(/\/setup (\w{8})/, (msg, match) => {
            ...
            if (promotable && match && match[1] === setupUUID) {
                promoteUser(promotable.id, -1).then(() => setupUUID = "")
...
export const promoteUser: (promotable: number, promoter: number) => Promise<unknown> = (promotable: number, promoter: number) => prisma.moderators.create({
    data: {
